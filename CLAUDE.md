...
- **Header is minimal (IA by stage, not by widget):** the header holds only the **Studio / Shelf** mode switch + a **⋯ settings** popover (theme; locale; **Export list / Share link** — the whole-scheme output actions; an **About & data** shortcut). Every other control lives in the stage it belongs to — the **Paints** drawer trigger + **seed hex** + **From photo** + **Compare** sit atop the Studio (the seed toolbar); the **seed's role** (Main|Accent) is set on the live palette itself via the **seed docks** (§3.5); the **Use my collection** control (off · prefer · only) lives in the Plan tab. Don't move *per-scheme/in-stage* controls into the header; the only header actions are the whole-output **Export/Share** (they act on the finished scheme, not a stage within it).
- **Top-level modes (maps to a future app tab bar):** a primary **Studio / Shelf** switch in the header. **Studio** is the scheme workspace (Paints drawer + studio + tabs). **Shelf** ("My paint shelf") is the full-width collection grid for stocking what you own / want to buy — bulk-toggled, Finder-style, with **search · status filter (owned/to-buy) · brand · type filter · sort** — the brand filter is a dropdown like the Paints drawer’s (the old wrapped chip rows cost three rows on a phone; `mockups/shelf-brand-filter.html`).
- **Paint picker is a header drawer, not a side column.** A **`☰ Paints`** button in the seed toolbar opens a full-width **drawer** that drops from the toolbar as an **overlay** (absolute → no reflow, §3.4; clip-path reveal, disabled under reduced motion). The drawer holds the **full** picker — **search · brand · type · sort**, **Import**/**Export CSV**, and a **horizontal** swatch strip (`#list.pstrip`, edge-masked so it scrolls within bounds, never bleeds). Each swatch carries its owned/to-buy **state badge**; **right-click** (or **P/U/X** on a focused chip) marks owned/to-buy/clear; click picks + closes. Picking is keyboard-operable (arrows/Home/End/Enter, Esc closes, focus returns to the trigger). The Shelf remains the full-screen grid for bulk collection work.
- **Layout (Studio):** a full-width **seed toolbar** (Paints · hex · From photo · Compare) tops the **single-column** workspace (max-width ~1440 — the picker is a drawer, so the workspace spans the full width). On wide screens (>1024px) the workspace **splits into two columns: a sticky *studio* column** (condensed **hero** line + harmony **wheel** + **live palette**) that stays in view, and a **tabbed output** column (**Plan·roles / Equivalents / Accessibility**) that scrolls beside it — the role plan's cards go **2-up** at this width. The role-mapped ~60-30-10 ideal-vs-actual output lives in the **Plan** tab. **One scheme, one colour bar:** the **live palette** is the single scheme summary — each column is labelled by its **role** (Primary/Secondary/Accent/Metal; Metal has no wheel node so it rides along as a **display-only** column), so it reads in the Plan's language. The Plan tab is the *detail* for that bar (ideal→nearest paint + tone ladders) and **must not** re-draw its own duplicate colour bar. The two columns are **one instrument, not two panels**: a **colour link** ties them — hovering/focusing a Plan role card rings the same colour's **wheel node + live-palette column** (and vice-versa), matched by `data-hex` (see §3.5). Keep them coupled — don't split the wheel and the plan into separate tabs/views.
- **Harmony control is a single scrollable strip** (not a wrapped multi-row block): the 10 schemes read as one horizontally scrollable list (`#seg.harmscroll`) on every width, so picking a harmony never reflows the studio. The active chip auto-centres.
- **Wheel module:** the hero above it is a **condensed one-line** identity strip (swatch chip · name · hex · seed pill · meta · buy), not a tall block. Each wheel node that plays a role wears a **P/A/2 badge** (`--accent`/`--on-accent` token pair, clamped inside the disc) with a **legend** below decoding it; the role is keyed off `schemeBase()` so it's correct in accent-seed mode, and `announceActive()` names it for screen readers. **Lightness** is a **vertical slider** in the wheel gutter + a compact **↻** Generate icon (a native, labelled, arrow-operable range; horizontal fallback ≤520px) — this reclaims the old control row.
- Breakpoints: **≤1024px** → the split collapses to one workspace column (the studio un-sticks and stacks above the output); **≤860px** → the studio stacks the wheel over the live palette; **≤520px** → compact paddings, the seed toolbar wraps (hex field full-width), the lightness slider returns to horizontal below the wheel, role slots stack, tabs scroll horizontally.
...
- `groups[]` is **auto-seeded at build time**: paints within ΔE 2000 ≤ 1.0 **and the same finish class** (metal · translucent wash/shade/ink/contrast/glaze · effect · opaque flat — a wash is never "interchangeable" with a flat that shares its hex, §2) are clustered (union-find) into equivalence groups (`{id, refHex, label}`), and each member paint carries a `groupId`. The Equivalents tab surfaces a paint's group ("interchangeable — same colour"); the *closest other-brand* matches are still computed at runtime by ΔE 2000. Groups are regenerated by `build-dataset.mjs`, then corrected by hand via `src/data/group-overrides.json` (relabel / split / exclude, anchored by stable paint ids).
...
- **Neutral seeds (v1.8).** A seed with Lab chroma **C\* < 10** (`isNeutral`; perceptual, so visually-black
  ...
  (the palette's Accent seed dock disables with the why — a neutral accent has no complement) and its Metal ideal is
