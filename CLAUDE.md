...
- **Theme switch (`#themeToggle`)**: the one sanctioned **slide-over toggle** — the §3.1 `◐` control — `role="switch"` with `aria-checked` = dark. A thumb slides between a sun (light) and moon (dark) with the icon cross-fading; the slide honours `prefers-reduced-motion` (state flips without animating). This is the *only* toggle-switch in the system; every other either/or picker (theme had been one too, language, harmony, shelf filters) stays a **segmented control** (`.seg`). Don't add more switch-style toggles without a line here.
